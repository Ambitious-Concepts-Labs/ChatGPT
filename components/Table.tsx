import { data, titles } from "../constants/tableData";
    ...
    <div className="w-full border-separate border-spacing-y-3">
      <div className="h-16 flex w-full mb-6 lg:mb-3">
        {titles.map((title, i) => (
          <div
            key={i}
            className={`w-1/3 lg:w-1/4 text-center font-semibold flex items-center justify-center lg:bg-transparent lg:text-black 
              ${
                i === 0
                  ? "hidden lg:block"
                  : i === 1
      ...
      <div className="flex flex-col gap-6 lg:gap-3 w-full">
        {data.map((feat, i) => (
          <div key={i}>
            <div className="text-center mb-6 text-sm font-semibold lg:hidden">
              {feat.title}
            </div>
