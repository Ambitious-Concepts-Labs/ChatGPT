...
export default function Popup() {
  ...
  const images = [avatar, logo, woman];
  ...
  return (
    <div className="fixed overflow-hidden w-screen flex flex-col-reverse z-20 gap-3 p-4 items-end justify-center bottom-0 right-0 md:bottom-1 md:px-4 md:w-2/4 md:py-2 lg:w-[28%] h-fit">
      <button
        onClick={handleOpen}
        className="bg-black text-white h-12 w-12 rounded-full flex items-center justify-center drop-shadow-xl cursor-pointer"
      >
        {isOpen ? (
      ...
      <div
        className={`grow shadow-xl rounded-xl w-full flex-col h-[90vh] pt-4 md:h-[80vh] md:pt-0 ${
          isOpen ? "flex" : "hidden"
        }`}
      >
        <div className="bg-zinc-800 rounded-t-xl p-2 pb-3 flex flex-col items-center gap-2">
          ...
          <ul className="flex">
            {images.map((image, i) => (
              <li
                key={i}
                className={`h-9 w-9 relative rounded-full overflow-hidden border-2 border-zinc-800 ${
                  i > 0 && "-ms-1.5"
                }`}
              >
                <Image
            ...
            <ul>
              <li className="flex gap-2">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="px-2.5 py-2 bg-zinc-800 text-white rounded-md text-xs">
                    How can we help with Writier?
                  </div>
                </div>
              </li>
            </ul>
