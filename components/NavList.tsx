             ...
             <li key={i} className='flex items-center justify-between'>
              {
                item.title === 'Support' ?
                (
                  <a onClick={() => setAskQuestion(!askQuestion)} href={item.src} className={`flex items-center gap-3 ${pathname !== item.src && 'text-slate-400'}`}>
                    <span>{item.icon}</span>
                    <span className='font-medium text-xs tracking-wide'>{item.title}</span>
                  </a>
                ) : (
                  <a href={item.src} className={`flex items-center gap-3 ${pathname !== item.src && 'text-slate-400'}`}>
                    <span>{item.icon}</span>
                    <span className='font-medium text-xs tracking-wide'>{item.title}</span>
                  </a>
                )
